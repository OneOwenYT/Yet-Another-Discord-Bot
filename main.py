...
import os #needed for the os.getenv() function to work and other directory related things
...
#    os.chdir(r'C:\Users\070LE\OneDrive\Desktop\code stuff\py\MTF Unit Proccesing bot\main.py') # Not needed as a VSC had a setting ticked to block py applications to access other files, this had block cogs from working but hadnt stopped the "import X from X" to not work :/ , **ONLY CHANGE THIS VVALUE IF YOU NEED IT**.
    ...
    print(f"The bot has came online at {formatTime} while running version {botVersion}.")
...
async def Commands(ctx):
    ...
    embedVar.add_field(name="Leave:", value="The bot will leave the VC.", inline=False)
    embedVar.add_field(name="Play:", value="Plays a audio file linked or attached (if its attached put anything after the command).", inline=False)
    await ctx.send(embed=embedVar)
...
async def Play(ctx, url):
    if ctx.voice_client:
        if url.startswith("http"):
            ...
            voice_client = ctx.voice_client
            source = discord.FFmpegPCMAudio(source=attachment)
            voice_client.play(source)

            await ctx.send("Playing attachment!")
        else:
